    ...
    try {
      ...
      if (result != null) {
        ...
        if (roleDT == "DOANHNGHIEP") {
          var resultOrder = await Order.findOneAndUpdate({ _id: idDonHangKhachHang, id_DoanhNghiep: IdCongTy }, update, {
            new: true
          });
          res.status(200).send({
            data: resultOrder,
  ...
  async ShowOrderByOneWeek(req, res, next) {
    try {
      ...
      if (result != null) {
        const roleDT = result.Role;
        if (roleDT == "DOANHNGHIEP") {
          var idCongTy = await (await DoanhNghiep.findOne({ id_account: result._doc._id }))._id;
          var ngayHienTai = new Date();
          ngayHienTai.setDate(ngayHienTai.getDate());
