                  ...
                  for (let i = 0; i < payment.links.length; i++) {
                    if (payment.links[i].rel === "approval_url") {
                      ...
                      if (soLuongDonHangDoanhNghiep > 0 && ngayThucTai <= ngayHetHanGoiDoanhNghiep) {
                        ...
                        await DoanhNghiep.findOneAndUpdate({ _id: update.id_DoanhNghiep }, resultDoanhNghiep._doc, {
                          new: true
                        });
                        res.status(200).send({
                          data: resultPayment,
                  ...
                  for (let i = 0; i < payment.links.length; i++) {
                    if (payment.links[i].rel === "approval_url") {
                      ...
                      if (soLuongDonHangDoanhNghiep > 0 && ngayThucTai <= ngayHetHanGoiDoanhNghiep) {
                        ...
                        await KhachHang.findOneAndUpdate({ id_account: _id }, resultKH._doc, {
                          new: true
                        });
                        res.status(200).send({
                          data: resultPayment,
  ...
  async XacNhanDonHang(req, res, next) {
    try {
      ...
      if (result != null) {
        const roleDT = result.Role;
        if (roleDT == "KHACHHANG") {
          var resultOrder = await Order.findOneAndUpdate({ _id: idDonHangKhachHang }, update, {
            new: true
          });
          res.status(200).send({
            data: resultOrder,
        ...
        if (roleDT == "KHACHHANG") {
          ...
          if (update.KhoiLuong > resultKH._doc.KhoiLuongToiDa || ngayThucTai > ngayHetHanGoiKhachHang || soLuongDonHangGoiKhachHang == 0) {
            ...
            if (update.KhoiLuong > khoiLuongBatBuoc) {
              ...
              chiPhiVanChuyen = parseFloat(chiPhiVanChuyen - chiPhiVanChuyen * tongGiamgia / 100);
              var resultCost = {};
              resultCost.TongGiaChuaGiam = tongGiaChuaGiam.toString();
              resultCost.SoTienGiamDuocDoGoi = soTienGiamDuocDoGoi.toString();
              resultCost.SoTienGiamDuocDoDoanhNghiep = SoTienGiamDuocDoDoanhNghiep.toString();
